...
export default function TrafficVolumeChart() {
  ...
  const options = {
    chart: { type: "area" as const, height: 180, toolbar: { show: false }, sparkline: { enabled: true } },
    colors: ["#10b981"],
    ...
    title: { text: "Traffic Volume Today", align: "left" as const, style: { fontSize: '12px md:text-14px', color: '#fff' } },
    tooltip: { theme: 'dark' as const }
  };

  return (
    <div className="w-full h-full">
      <Chart options={options} series={series} height={180} />
    </div>
  );
}