...
// کارکرد: هشدار بزرگ + نمودار حوادث + لیست Recent Incidents
// ویژگی جدید: هشدار بزرگ + نمودار خطی + لیست حوادث اخیر
...
export default function SecurityPage() {
  ...
  const options = {
    ...
    tooltip: {
      theme: 'dark' as const,
      y: { 
        formatter: (val: number) => `${val} incidents`  // رفع خطا
      }
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {/* Active Incidents — بزرگ‌تر و هشدار قرمز */}
        <div className="bg-gradient-to-br from-red-600 to-pink-700 rounded-xl p-8 text-white text-center">
          <AlertTriangle className="w-16 h-16 mx-auto mb-4 opacity-90" />
          <p className="text-7xl font-bold">7</p>
          <p className="text-lg opacity-90 mt-2">Active Incidents</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {recentIncidents.map((incident, index) => (
            <div key={index} className="bg-white/10 backdrop-blur rounded-xl p-6 border border-white/20">
              ...
              <p className="text-lg font-bold text-white mb-2">{incident.type}</p>
